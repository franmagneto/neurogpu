...
bool ReallocateOutputs(MLPNetwork *nnet) {
	...
	for (int i = 1; i < nnet->nLayers; ++i) {
		nnet->layers[i]->d_outs = allocateFloatsDev(
				nnet->layers[i]->nNeurons * nnet->nCases);
		if (nnet->layers[i]->d_outs == NULL) {
			FreeOutputs(nnet);
			return false;
		}
...
bool ReallocateDeltas(MLPNetwork *nnet) {
	...
	for (int i = 1; i < nnet->nLayers; ++i) {
		nnet->layers[i]->d_deltas = allocateFloatsDev(
				nnet->layers[i]->nNeurons * nnet->nCases);
		if (nnet->layers[i]->d_deltas == NULL) {
			FreeDeltas(nnet);
			return false;
...
void DestroyDataSet(DataSet *dset) {
	...
	if (dset->location == LOC_HOST) {
		if (dset->d_inputs != NULL || dset->d_outputs != NULL) {
			fprintf(stderr,
					"Location of dataset is HOST but device ptrs are not NULL\n");
			exit(-1);
		}
	} else {
...
bool TransferDataSetToDevice(DataSet *data) {
	...
	if (data->location == LOC_HOST) {
		...
		e = cudaMemcpy(data->d_inputs, data->inputs, nFloatsIn * sizeof(float),
				cudaMemcpyHostToDevice);

		if (e != cudaSuccess) {
			fprintf(stderr,
					"Error copying dataset inputs from host to device: %s\n",
		...
		if (e != cudaSuccess) {
			fprintf(stderr,
					"Error copying dataset outputs from host to device: %s\n",
					cudaGetErrorString(e));
			return false;
		}
...
void PresentInputsFromDataSet(MLPNetwork *nnet, DataSet *dset, int actf,
	...
	PresentInputs(nnet, dset->d_inputs, actf, casesPerBlock, neuronsPerThread);
}
...
void PresentInputs(MLPNetwork *nnet, float *d_inputs, int actf,
	...
	if (casesPerBlock > nnet->nCases)
		casesPerBlock = nnet->nCases;

	int max_blocks = nnet->nCases / casesPerBlock;
	int min_blocks = nnet->nCases % casesPerBlock;
	min_blocks = min_blocks ? min_blocks : max_blocks;

	for (int l = 1; l < nnet->nLayers; ++l) {
		if (neuronsPerThread > nnet->layers[l]->nNeurons)
			neuronsPerThread = nnet->layers[l]->nNeurons;
		int max_threads = nnet->layers[l]->nNeurons / neuronsPerThread;
		int min_threads = nnet->layers[l]->nNeurons % neuronsPerThread;
		min_threads = min_threads ? min_threads : max_threads;

		for (int i = 0; i < casesPerBlock; ++i) {
			int blocks = (i == casesPerBlock-1) ? min_blocks : max_blocks;
...
__global__ void deltas_hlayer(float *outs, float *d_weights, float *d_deltas,
		float *d_dltnext, int neuronsNext, int nxtLayerWOffset,
		int weightsPerNeuronNxt) {
	...
	for (int i = 0; i < neuronsNext; ++i, wid += weightsPerNeuronNxt)
		d_deltas[tid] += d_weights[wid] * d_dltnext[oid + i]
				* dsigmoid(outs[tid]);
}
...
__global__ void derivs_layer(float *d_inputs, float *d_deltas, float *d_derivs,
		int nNeurons, int neuronsPrev, int nWeights, int weightsPerNeuron,
		int weightOffset) {
	// weight index
...
__global__ void update_weights_nreduc(float *d_weights, float *d_derivs,
		float lrate, int nCases, int nWeights) {
	float dE = 0.0f;
...
float BatchTrainBackprop(MLPNetwork *nnet, DataSet *data, int epochs,
	...
	if (actf != ACTF_SIGMOID && actf != ACTF_TANH)
		return -1.0f;
	...
	if (d_err == NULL) {
		fprintf(stderr,
				"Error allocating memory on device to store errors: %s\n",
	...
	for (int e = 0; e < epochs; ++e) {
		...
		PresentInputs(nnet, data->d_inputs, actf, casesPerBlock,
				neuronsPerThread);

		// backpropagation: calculation of deltas
		deltas_output<<<data->nCases, nOutputs>>>(outLayer->d_outs,
				data->d_outputs, outLayer->d_deltas, d_err);
		...
		for (int l = nnet->nLayers - 2; l > 0; --l) {
			...
			deltas_hlayer<<<data->nCases, layer->nNeurons>>>(layer->d_outs,
					nnet->d_weights, layer->d_deltas, nextLayer->d_deltas,
					nextLayer->nNeurons, nextLayer->weightOffset,
		...
		for (int l = 1; l < nnet->nLayers; ++l) {
			...
			derivs_layer<<<data->nCases, nw>>>(prevLayer->d_outs,
					layer->d_deltas, d_derivs, layer->nNeurons,
					prevLayer->nNeurons, nnet->nWeights,
					layer->weightsPerNeuron, layer->weightOffset);
			prevLayer = layer;
		}

		// update weights based on derivatives
		update_weights_nreduc<<<1, nnet->nWeights>>>(nnet->d_weights, d_derivs,
				lrate, data->nCases, nnet->nWeights);
	}
...
void PrintWeights(MLPNetwork *nnet) {
	...
	if (h_weights == NULL) {
		...
		e = cudaMemcpy(h_weights, nnet->d_weights,
				nnet->nWeights * sizeof(float), cudaMemcpyDeviceToHost);
...
float *GetLayerOutputs(MLPNetwork *nnet, int ixLayer) {
	...
	cudaMemcpy(result, nnet->layers[ixLayer]->d_outs, length * sizeof(float),
			cudaMemcpyDeviceToHost);
